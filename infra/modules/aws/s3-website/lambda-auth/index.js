...
function mainProcess(event, context, callback) {
  ...
  const queryDict = new URLSearchParams(request.querystring);
  ...
  if (request.uri.startsWith(config.CALLBACK_PATH)) {
    ...
    const errorResponse = queryDict.get('error');
    if (errorResponse) {
      const errors = {
      ...
      if (errorResponse_description != null) {
        error_description = errorResponse_description;
      } else {
        error_description = '';
      }

      if (errorResponse_uri != null) {
        error_uri = errorResponse_uri;
      } else {
        error_uri = '';
      }
    ...
    const code = queryDict.get('code')
    if (!code) {
      unauthorized('No Code Found', '', '', callback);
    }
    config.TOKEN_REQUEST.code = code;
                ...
                const response = {
                  ...
                  "headers": {
                    "location" : [
                      {
                        "key": "Location",
                        "value": event.Records[0].cf.config.hasOwnProperty('test') ? (config.AUTH_REQUEST.redirect_uri + queryDict.get('state')) : queryDict.get('state')
                      }
                    ],
                    "set-cookie" : [
                      {
